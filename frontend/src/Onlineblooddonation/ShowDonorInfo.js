...
function ShowDonorInfo() {
  ...
  const getData = async () => {
    ...
    const response = await fetch('https://online-blood-donation-backend.onrender.com/getDonorsByEmail/'+email);
  ...
  const deleteDonor = async (id) => {
    ...
    if (window.confirm("are you sure want to delete?")); {
      ...
      const response = await fetch(`https://online-blood-donation-backend.onrender.com/delete/${id}`, requestOptions);
